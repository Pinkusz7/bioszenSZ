              ...
              tabPanel("Gráficos & Stats",
                       ...
                       sidebarLayout(
                         ...
                         sidebarPanel(
                           ...
                           fileInput('dataFile',  'Cargar metadata-parametros (.xlsx)', accept = '.xlsx'),
...
server <- function(input, output, session) {
  ...
  observeEvent(
         ...
         input$doNorm, input$ctrlMedium),
    {
      ...
      updateNumericInput(session, "xmax_corr",
                         value = cfg_x$Y_Max, max = Inf)
      updateNumericInput(session, "xbreak_corr",
                         value = cfg_x$Interval, min = 0.0001)
      updateNumericInput(session, "xmin_corr", value = 0)
      updateNumericInput(session, "ymin_corr", value = 0)
      updateNumericInput(session, "ymax_corr",
                         value = cfg_y$Y_Max, max = Inf)
      updateNumericInput(session, "ybreak_corr",
                         value = cfg_y$Interval, min = 0.0001)
    },
    ignoreInit = FALSE
  )
  ...
  build_plot <- function(scope, strain = NULL, tipo) {  
    ...
    if (tipo == "Correlación") {
      ...
      p <- ggplot(df, aes(X, Y)) +
        ...
        scale_x_continuous(
          limits = c(input$xmin_corr, input$xmax_corr),
          breaks = seq(input$xmin_corr, input$xmax_corr,
                       by = input$xbreak_corr),
          expand = c(0, 0)
        ) +
        scale_y_continuous(
          limits = c(input$ymin_corr, input$ymax_corr),
          breaks = seq(input$ymin_corr, input$ymax_corr,
                       by = input$ybreak_corr),
          expand = c(0, 0)
        ) +
        coord_cartesian(clip = "off") +
        theme_minimal() +
  ...
  output$downloadMetadata <- downloadHandler(
    ...
    content  = function(file){
      ...
      if (input$tipo %in% c("Boxplot", "Barras")){
        ...
        # copia toda la Sheet2 del archivo de curvas
